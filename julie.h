...
#define _JULIE_STATUS                                                                                                                           \
    ...
    _JULIE_STATUS_X(JULIE_ERR_ERROR_VALUE,                     "Error value thrown.")                                                           \
    _JULIE_STATUS_X(JULIE_ERR_ACTOR_DEAD,                      "Actor is no longer alive.")                                                     \
    _JULIE_STATUS_X(JULIE_ERR_ACTOR_JOIN,                      "Actor join failed.")                                                            \
    _JULIE_STATUS_X(JULIE_ERR_NO_MAILBOX,                      "No mailbox available in this context.")
...
#define _JULIE_TYPE                                                                    \
    ...
    _JULIE_TYPE_X(JULIE_ERROR,             "error")                                    \
    _JULIE_TYPE_X(JULIE_ACTOR,             "actor")                                    \
...
typedef struct Julie_Mailbox_Struct       Julie_Mailbox;
typedef struct Julie_Actor_Struct         Julie_Actor;
...
struct Julie_Thrown_Error_Info_Struct {
    Julie_Value *error_value;
};
...
typedef struct Julie_Thrown_Error_Info_Struct Julie_Thrown_Error_Info;
...
struct Julie_Error_Info_Struct {
    ...
    union {
        ...
        Julie_Thrown_Error_Info                 thrown;
    };
};
...
typedef Julie_Status (*Julie_Eval_Callback)(Julie_Interp *interp, Julie_Value *value, Julie_Value **result);
...
Julie_Status julie_parse_roots(Julie_Interp *interp, Julie_Array **rootsp, const char *str, int size, unsigned long long *err_line, unsigned long long *err_col, int temporary);
...
void julie_make_runtime_error(Julie_Interp *interp, Julie_Value *expr, Julie_Value *error_value);
...
Julie_Value *julie_error_value(Julie_Interp *interp, const char *error_message);
...
#define ARRAY_FOR_EACH(_arrayp, _it)                                                                                \
    for (unsigned long long _each_i = 0;                                                                            \
         ((_arrayp) != NULL && _each_i < (_arrayp)->len && (((_it) = (__typeof(_it))(_arrayp)->data[_each_i]), 1)); \
         _each_i += 1)
...
typedef struct {
    char        *name;
    Julie_Value *value;
} Julie_Detached_Capture;

typedef struct {
    char        *name;
    Julie_Value *value;
} Julie_Detached_Binding;

typedef struct {
    char        *cur_file;
    Julie_Array *captures;
} Julie_Detached_Closure_Info;

struct Julie_Actor_Struct {
    pthread_t                 thread;
    Julie_Mailbox             mailbox;
    volatile int              alive;
    volatile int              rc;
    volatile int              stop;
    int                       joined;
    int                       is_main;
    Julie_Value              *detached_result;
    Julie_Status              final_status;
    Julie_Value              *detached_error_value;
    Julie_Value              *detached_expr;
    Julie_Array              *bindings;
    Julie_Actor              *parent;
...
#endif
    pthread_mutex_t           children_mtx;
    Julie_Array              *children;
};

static void julie_actor_ref(Julie_Actor *actor);
static void julie_actor_unref(Julie_Actor *actor);
static void julie_free_detached_value(Julie_Value *value);
...
static int julie_equal(Julie_Value *a, Julie_Value *b) {
    ...
    Julie_Value         *ia;
    Julie_Value         *ib;
    Julie_Closure_Info  *closurea;
    Julie_Closure_Info  *closureb;
    Julie_String_ID      sym;
    Julie_Value        **val;
    Julie_Value        **lookup;
    Julie_Value         *key;
    ...
    switch (a->type) {
        ...
        case JULIE_LAMBDA:
            ...
            if (a->type == JULIE_LAMBDA) {
                closurea = julie_array_get_aux(a->list);
                closureb = julie_array_get_aux(b->list);

                hash_table_traverse(closurea->captures, sym, val) {
                    lookup = hash_table_get_val(closureb->captures, sym);
                    if (lookup == NULL || !julie_equal(*val, *lookup)) { return 0; }
                }
            }
        ...
        case JULIE_OBJECT:
        case JULIE_ERROR:
            hash_table_traverse((_Julie_Object)a->object, key, val) {
                lookup = hash_table_get_val((_Julie_Object)b->object, key);
                if (lookup == NULL || !julie_equal(*val, *lookup)) { return 0; }
            }
            return 1;
        case JULIE_BUILTIN_FN:
            return a->builtin_fn == b->builtin_fn;
        case JULIE_ACTOR:
...
static Julie_Value *julie_detach_value(Julie_Value *value) {
    Julie_Value                  *copy;
    Julie_Source_Value_Info      *source_info;
    Julie_Source_Value_Info      *new_source_info;
    Julie_Value                  *it;
    Julie_Value                  *key;
    Julie_Value                 **val;
    Julie_Closure_Info           *closure;
    Julie_Detached_Closure_Info  *dclosure;
    Julie_Detached_Binding       *new_binding;
    Julie_String_ID               sym;

    copy = malloc(sizeof(Julie_Value));
    *copy              = *value;
    copy->owned        = 0;
    copy->borrow_count = 0;
    copy->source_node  = 0;
    ...
    switch (value->type) {
        case JULIE_STRING:
        case JULIE_SYMBOL:
            copy->cstring = strdup(julie_value_cstring(value));
            copy->tag     = JULIE_STRING_TYPE_MALLOC;
            break;
        ...
        case JULIE_LIST:
            ...
            JULIE_ARRAY_RESERVE(copy->list, julie_array_len(value->list));
            ARRAY_FOR_EACH(value->list, it) {
                JULIE_ARRAY_PUSH(copy->list, julie_detach_value(it));
            }
        ...
        case JULIE_OBJECT:
        case JULIE_ERROR:
            copy->object = hash_table_make_e(Julie_Value_Ptr, Julie_Value_Ptr,
                                             julie_value_hash, julie_equal);
            hash_table_traverse((_Julie_Object)value->object, key, val) {
                hash_table_insert((_Julie_Object)copy->object, julie_detach_value(key), julie_detach_value(*val));
            }
            break;
        ...
        case JULIE_LAMBDA:
            ...
            closure  = julie_array_get_aux(value->list);
            dclosure = malloc(sizeof(*dclosure));

            dclosure->cur_file = closure->cur_file
                ? strdup(julie_get_cstring(closure->cur_file))
                : NULL;

            dclosure->captures = JULIE_ARRAY_INIT;
            hash_table_traverse(closure->captures, sym, val) {
                new_binding        = malloc(sizeof(*new_binding));
                new_binding->name  = strdup(julie_get_cstring(sym));
                new_binding->value = julie_detach_value(*val);
                JULIE_ARRAY_PUSH(dclosure->captures, new_binding);
            }

            JULIE_ARRAY_SET_AUX(copy->list, dclosure);
            break;

        case JULIE_ACTOR:
            julie_actor_ref(copy->actor);
            break;
    }
...
static Julie_Value *_julie_attach_value(Julie_Interp *dst, Julie_Value *detached, int source_node) {
    ...
    Julie_Detached_Closure_Info  *dclosure;
    Julie_Closure_Info           *closure;
    Julie_Detached_Binding       *dbinding;
    ...
    *copy              = *detached;
    ...
    switch (detached->type) {
        ...
        case JULIE_SYMBOL:
            ...
            free(detached->cstring);
            break;

        case JULIE_LIST:
            copy->list = JULIE_ARRAY_INIT;
            JULIE_ARRAY_RESERVE(copy->list, julie_array_len(detached->list));
            ARRAY_FOR_EACH(detached->list, it) {
                JULIE_ARRAY_PUSH(copy->list, _julie_attach_value(dst, it, source_node));
            }
            ...
            julie_array_free(detached->list);
            break;
        ...
        case JULIE_ERROR:
            copy->object = hash_table_make_e(Julie_Value_Ptr, Julie_Value_Ptr,
                                             julie_value_hash, julie_equal);
            hash_table_traverse((_Julie_Object)detached->object, key, val) {
                hash_table_insert((_Julie_Object)copy->object, _julie_attach_value(dst, key, source_node), _julie_attach_value(dst, *val, source_node));
            }
            hash_table_free((_Julie_Object)detached->object);
            break;
        ...
        case JULIE_LAMBDA:
            ...
            dclosure = (Julie_Detached_Closure_Info*)
                            julie_array_get_aux(detached->list);
            ...
            closure->cur_file = dclosure->cur_file
                ? julie_get_string_id(dst, dclosure->cur_file)
                : NULL;
            closure->captures = hash_table_make(Julie_String_ID, Julie_Value_Ptr, julie_string_id_hash);

            ARRAY_FOR_EACH(dclosure->captures, dbinding) {
                hash_table_insert(closure->captures,
                                  julie_get_string_id(dst, dbinding->name),
                                  _julie_attach_value(dst, dbinding->value, source_node));

                free(dbinding->name);
                free(dbinding);
            }
            julie_array_free(dclosure->captures);
            if (dclosure->cur_file != NULL) {
                free(dclosure->cur_file);
            }
            free(dclosure);

            JULIE_ARRAY_SET_AUX(copy->list, closure);
            break;

        case JULIE_ACTOR:
            /* refcount was already bumped during detach */
            break;
    }
...
static Julie_Value *julie_attach_value(Julie_Interp *dst, Julie_Value *detached) {
    return _julie_attach_value(dst, detached, 0);
}

static Julie_Value *julie_attach_source_value(Julie_Interp *dst, Julie_Value *detached) {
    return _julie_attach_value(dst, detached, 1);
}
...
static void julie_free_detached_value(Julie_Value *value) {
    ...
    switch (value->type) {
        ...
        case JULIE_ERROR:
            hash_table_traverse((_Julie_Object)value->object, key, val) {
                julie_free_detached_value(key);
                julie_free_detached_value(*val);
            }
        ...
        case JULIE_LAMBDA:
            dclosure = (Julie_Detached_Closure_Info*)
                            julie_array_get_aux(value->list);
            if (dclosure != NULL) {
                ARRAY_FOR_EACH(dclosure->captures, dbinding) {
                    free(dbinding->name);
                    julie_free_detached_value(dbinding->value);
                    free(dbinding);
                }
                julie_array_free(dclosure->captures);
                if (dclosure->cur_file != NULL) {
        ...
        case JULIE_ACTOR:
            julie_actor_unref((Julie_Actor*)value->object);
            break;
...
Julie_Value *julie_error_value(Julie_Interp *interp, const char *error_message) {
    ...
    key     = julie_symbol_value(interp, julie_get_string_id(interp, "'__message__"));
    message = julie_string_value(interp, error_message);
    julie_object_insert_field(interp, v, key, message, NULL);

    key = julie_symbol_value(interp, julie_get_string_id(interp, "'__backtrace__"));

    backtrace = julie_list_value(interp);
    ...
    while ((it = julie_bt_entry(interp, i)) != NULL) {
        ...
        snprintf(buff, sizeof(buff), "%s:%llu:%llu %s",
                 ...
                 s);
        free(s);
        JULIE_ARRAY_PUSH(backtrace->list, julie_string_value(interp, buff));

        i += 1;
    }

    if (!interp->actor->is_main) {
        snprintf(buff, sizeof(buff), "<actor %p>", (void*)interp->actor);
        JULIE_ARRAY_PUSH(backtrace->list, julie_string_value(interp, buff));
    }

    julie_object_insert_field(interp, v, key, backtrace, NULL);
...
object_print:;
        ...
        case JULIE_LAMBDA:
            ...
            break;
        case JULIE_ACTOR:
            snprintf(b, sizeof(b), "<actor %p>", (void*)value->actor);
            PUSHS(b);
            break;
        default:
            JULIE_ASSERT(0);
...
static Julie_Status _julie_bind_new(Julie_Interp           *interp,
    ...
    JULIE_ASSERT(!ref || (*valuep)->owned);

    need_copy = !ref;

    if (!ref) {
        if (!(*valuep)->owned && !(*valuep)->source_node) {
            need_copy = 0;
        }
    }
...
#define FREE_IF_NOT_NULL(x) if ((x) != NULL) { free(x); }
    ...
    switch (info->status) {
        ...
        case JULIE_ERR_REGEX:
            ...
            break;
        case JULIE_ERR_ERROR_VALUE:
            julie_free_value(info->interp, info->thrown.error_value);
            break;
        default:
            break;
    }
...
void julie_make_runtime_error(Julie_Interp *interp, Julie_Value *expr, Julie_Value *error_value) {
    ...
    memset(&info, 0, sizeof(info));
    info.interp                = interp;
    info.status                = JULIE_ERR_ERROR_VALUE;

    info.thrown.error_value = julie_force_copy(interp, error_value);
...
static Julie_Status julie_parse_next_value(Julie_Parse_Context *cxt, Julie_Value **valout, Julie_Token *tkout) {
    ...
    *tkout  = JULIE_TK_NONE;
    ...
    if (tk == JULIE_TK_LPAREN) {
        top = julie_array_top(cxt->parse_stack);
        ...
        if (status != JULIE_SUCCESS) {
            julie_free_value(cxt->interp, val);
    ...
    switch (tk) {
        case JULIE_TK_SYMBOL:
            if (tk_end - tk_start == 3 && strncmp(tk_start, "nil", tk_end - tk_start) == 0) {
                val = cxt->temporary
                        ? julie_nil_value(cxt->interp)
                        : julie_source_nil_value(cxt->interp);
            } else {
                sbuff = alloca(tk_end - tk_start + 1);
                memcpy(sbuff, tk_start, tk_end - tk_start);
                sbuff[tk_end - tk_start] = 0;
                val = cxt->temporary
                        ? julie_symbol_value(cxt->interp, julie_get_string_id(cxt->interp, sbuff))
                        : julie_source_symbol_value(cxt->interp, julie_get_string_id(cxt->interp, sbuff));
            }
            break;
...
add_char:;
            ...
            val = cxt->temporary
                    ? julie_interned_string_value(cxt->interp, julie_get_string_id(cxt->interp, sbuff))
                    : julie_source_interned_string_value(cxt->interp, julie_get_string_id(cxt->interp, sbuff));
            break;
        case JULIE_TK_SINT:
            ...
            val = cxt->temporary
                    ? julie_sint_value(cxt->interp, s)
                    : julie_source_sint_value(cxt->interp, s);
            break;
        case JULIE_TK_HEX:
            strncpy(tk_copy, tk_start, tk_end - tk_start);
            tk_copy[tk_end - tk_start] = 0;
            ...
            val = cxt->temporary
                    ? julie_uint_value(cxt->interp, u)
                    : julie_source_uint_value(cxt->interp, u);
            break;
        case JULIE_TK_FLOAT:
            strncpy(tk_copy, tk_start, tk_end - tk_start);
            tk_copy[tk_end - tk_start] = 0;
            sscanf(tk_copy, "%lg", &d);
            val = cxt->temporary
                    ? julie_float_value(cxt->interp, d)
                    : julie_source_float_value(cxt->interp, d);
            break;
...
static Julie_Status julie_builtin_typeof(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    ...
    *result = NULL;
    ...
    status = julie_eval_no_throw(interp, values[0], &val);
    if (status != JULIE_SUCCESS) {
        *result = NULL;
        goto out;
    }
...
static Julie_Status julie_builtin_error(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *message;
    ...
    *result = NULL;
    ...
    if (message->type != JULIE_STRING) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, message, JULIE_STRING, message->type);
        goto out_free_message;
    }

    *result = julie_error_value(interp, julie_value_cstring(message));
...
static Julie_Status julie_builtin_error_field(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *error;
    Julie_Value  *key;
    Julie_Value  *field;
    ...
    status = julie_eval_no_throw(interp, values[0], &error);
    if (status != JULIE_SUCCESS) {
        *result = NULL;
        goto out;
    }

    if (error->type != JULIE_ERROR) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[0], JULIE_ERROR, error->type);
        *result = NULL;
        goto out_free_error;
    }
    ...
    field = julie_object_get_field(error, key);

    if (field == NULL) {
        status = JULIE_ERR_BAD_INDEX;
        julie_make_bad_index_error(interp, values[1], key);
        *result = NULL;
        goto out_free_key;
    } else {
        field->owned = error->owned;
...
static Julie_Status _julie_builtin_assign(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result, int global) {
    ...
    *result = NULL;
    ...
    l = values[0];
    status = julie_eval_no_throw(interp, values[1], &rval);
    ...
    if (l->type == JULIE_SYMBOL) {
        ...
        if (julie_borrows_to_subvalues_outstanding(lval, lval)) {
            *result = NULL;
            status = JULIE_ERR_RELEASE_WHILE_BORROWED;
            julie_make_bind_error(interp, expr, status, NULL);
            julie_free_value(interp, rval);
            goto out;
        }
...
static Julie_Status julie_builtin_unbind(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status     status;
    Julie_Value     *sym;
    Julie_String_ID  id;
    Julie_Value     *lookup;
...
static inline double julie_value_to_double(Julie_Value *v) {
    if (v->type == JULIE_SINT) { return (double)v->sint; }
    if (v->type == JULIE_UINT) { return (double)v->uint; }
    return v->floating;
}

static Julie_Status julie_builtin_pow(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *base;
    Julie_Value  *exponent;
    ...
    *result = NULL;
    ...
    status = julie_eval(interp, values[0], &base);
    if (status != JULIE_SUCCESS) {
        goto out;
    }

    if (!JULIE_TYPE_IS_NUMBER(base->type)) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[0], _JULIE_NUMBER, base->type);
        julie_free_value(interp, base);
        goto out;
    }

    status = julie_eval(interp, values[1], &exponent);
    if (status != JULIE_SUCCESS) {
        julie_free_value(interp, base);
        goto out;
    }

    if (!JULIE_TYPE_IS_NUMBER(exponent->type)) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[1], _JULIE_NUMBER, exponent->type);
        julie_free_value(interp, base);
        julie_free_value(interp, exponent);
        goto out;
    }

    *result = julie_float_value(interp, pow(julie_value_to_double(base), julie_value_to_double(exponent)));
    julie_free_value(interp, base);
    julie_free_value(interp, exponent);
...
static Julie_Status _julie_builtin_math1(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result, double (*fn)(double)) {
    Julie_Status  status;
    Julie_Value  *v;
    ...
    *result = NULL;
    ...
    status = julie_eval(interp, values[0], &v);
    if (status != JULIE_SUCCESS) {
        goto out;
    }

    if (!JULIE_TYPE_IS_NUMBER(v->type)) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[0], _JULIE_NUMBER, v->type);
        julie_free_value(interp, v);
        goto out;
    }

    *result = julie_float_value(interp, fn(julie_value_to_double(v)));
    julie_free_value(interp, v);
...
static Julie_Status julie_builtin_abs(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    return _julie_builtin_math1(interp, expr, n_values, values, result, fabs);
}

static Julie_Status julie_builtin_sqrt(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    return _julie_builtin_math1(interp, expr, n_values, values, result, sqrt);
}

static Julie_Status julie_builtin_floor(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    return _julie_builtin_math1(interp, expr, n_values, values, result, floor);
}

static Julie_Status julie_builtin_ceil(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    return _julie_builtin_math1(interp, expr, n_values, values, result, ceil);
}

static Julie_Status julie_builtin_round(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    return _julie_builtin_math1(interp, expr, n_values, values, result, round);
}

static Julie_Status julie_builtin_log(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    return _julie_builtin_math1(interp, expr, n_values, values, result, log);
}

static Julie_Status julie_builtin_log2(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    return _julie_builtin_math1(interp, expr, n_values, values, result, log2);
}

static Julie_Status julie_builtin_log10(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    return _julie_builtin_math1(interp, expr, n_values, values, result, log10);
}

static Julie_Status julie_builtin_exp(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    return _julie_builtin_math1(interp, expr, n_values, values, result, exp);
}

static Julie_Status julie_builtin_sin(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    return _julie_builtin_math1(interp, expr, n_values, values, result, sin);
}

static Julie_Status julie_builtin_cos(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    return _julie_builtin_math1(interp, expr, n_values, values, result, cos);
}

static Julie_Status julie_builtin_tan(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    return _julie_builtin_math1(interp, expr, n_values, values, result, tan);
}

static Julie_Status julie_builtin_asin(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    return _julie_builtin_math1(interp, expr, n_values, values, result, asin);
}

static Julie_Status julie_builtin_acos(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    return _julie_builtin_math1(interp, expr, n_values, values, result, acos);
}

static Julie_Status julie_builtin_atan(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    return _julie_builtin_math1(interp, expr, n_values, values, result, atan);
}

static Julie_Status julie_builtin_atan2(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *y;
    Julie_Value  *x;
    ...
    *result = NULL;
    ...
    status = julie_eval(interp, values[0], &y);
    if (status != JULIE_SUCCESS) {
        goto out;
    }

    if (!JULIE_TYPE_IS_NUMBER(y->type)) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[0], _JULIE_NUMBER, y->type);
        julie_free_value(interp, y);
        goto out;
    }

    status = julie_eval(interp, values[1], &x);
    if (status != JULIE_SUCCESS) {
        julie_free_value(interp, y);
        goto out;
    }

    if (!JULIE_TYPE_IS_NUMBER(x->type)) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[1], _JULIE_NUMBER, x->type);
        julie_free_value(interp, y);
        julie_free_value(interp, x);
        goto out;
    }

    *result = julie_float_value(interp, atan2(julie_value_to_double(y), julie_value_to_double(x)));
    julie_free_value(interp, y);
    julie_free_value(interp, x);
...
static Julie_Status julie_builtin_clamp(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *val;
    Julie_Value  *lo;
    Julie_Value  *hi;
    ...
    *result = NULL;
    ...
    status = julie_eval(interp, values[1], &lo);
    if (status != JULIE_SUCCESS) {
        julie_free_value(interp, val);
        goto out;
    }

    if (!JULIE_TYPE_IS_NUMBER(lo->type)) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[1], _JULIE_NUMBER, lo->type);
        julie_free_value(interp, val);
        julie_free_value(interp, lo);
        goto out;
    }

    status = julie_eval(interp, values[2], &hi);
    if (status != JULIE_SUCCESS) {
        julie_free_value(interp, val);
        julie_free_value(interp, lo);
        goto out;
    ...
    if (!JULIE_TYPE_IS_NUMBER(hi->type)) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[2], _JULIE_NUMBER, hi->type);
        julie_free_value(interp, val);
        julie_free_value(interp, lo);
        julie_free_value(interp, hi);
        goto out;
    }

    dval    = julie_value_to_double(val);
    dlo     = julie_value_to_double(lo);
    dhi     = julie_value_to_double(hi);
    *result = julie_float_value(interp, dval < dlo ? dlo : dval > dhi ? dhi : dval);
...
static Julie_Status julie_builtin_list(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    ...
    for (i = 0; i < n_values; i += 1) {
        it     = values[i];
        status = julie_eval_no_throw(interp, it, &ev);
        if (status != JULIE_SUCCESS) {
            *result = NULL;
            goto out_free;
        }
...
static Julie_Status julie_builtin_insert(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    ...
    *result = NULL;
    ...
    status = julie_eval_no_throw(interp, values[1], &val);
    if (status != JULIE_SUCCESS) {
        *result = NULL;
        goto out_free_list;
    }

    status = julie_eval(interp, values[2], &idx);
    if (status != JULIE_SUCCESS) {
        *result = NULL;
        julie_free_value(interp, val);
        goto out_free_list;
    ...
    if (!JULIE_TYPE_IS_INTEGER(idx->type)) {
        *result = NULL;
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[2], _JULIE_INTEGER, idx->type);
        julie_free_value(interp, val);
        goto out_free_list;
    }
...
static Julie_Status julie_builtin_map(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *fn;
    Julie_Value  *list;
    Julie_Value  *mapped;
    ...
    *result = NULL;
    ...
    status = julie_eval(interp, values[0], &fn);
    if (status != JULIE_SUCCESS) {
        goto out;
    }

    status = julie_eval(interp, values[1], &list);
    if (status != JULIE_SUCCESS) {
        julie_free_value(interp, fn);
        goto out;
    }
    ...
    mapped = julie_list_value(interp);

    JULIE_ARRAY_PUSH(interp->iter_vals, list);

    ARRAY_FOR_EACH(list->list, it) {
        status = julie_invoke(interp, values[0], fn, 1, &it, &ev);
        if (status != JULIE_SUCCESS) {
        ...
        JULIE_ARRAY_PUSH(mapped->list, ev);
    }
...
static Julie_Status julie_builtin_filter(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *fn;
    Julie_Value  *list;
    ...
    *result = NULL;
    ...
    filtered = julie_list_value(interp);
    ...
    ARRAY_FOR_EACH(list->list, it) {
        ...
        if (!JULIE_TYPE_IS_INTEGER(ev->type)) {
            status = JULIE_ERR_TYPE;
            julie_make_type_error(interp, ev, _JULIE_INTEGER, ev->type);
            julie_free_value(interp, ev);
            julie_free_value(interp, filtered);
            goto out_pop;
        }

        truth = (ev->type == JULIE_SINT)
                    ? !!ev->sint
                    : !!ev->uint;
        ...
        if (truth) {
            JULIE_ARRAY_PUSH(filtered->list, julie_force_copy(interp, it));
        }
    }
...
static Julie_Status julie_builtin_reduce(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *fn;
    Julie_Value  *acc;
    Julie_Value  *list;
    ...
    *result = NULL;
    ...
    status = julie_eval(interp, values[1], &acc);
    if (status != JULIE_SUCCESS) {
        julie_free_value(interp, fn);
        goto out;
    }

    status = julie_eval(interp, values[2], &list);
    if (status != JULIE_SUCCESS) {
    ...
    if (list->type != JULIE_LIST) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[2], JULIE_LIST, list->type);
        goto out_free;
    }
    ...
    ARRAY_FOR_EACH(list->list, it) {
        arg_pass[0] = acc;
        arg_pass[1] = it;
        status = julie_invoke(interp, values[0], fn, 2, arg_pass, &ev);
        if (status != JULIE_SUCCESS) {
            julie_free_value(interp, acc);
            goto out_pop;
        }
...
static Julie_Status julie_builtin_reverse(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *list;
    Julie_Value  *rev;
    long long     i;
    ...
    *result = NULL;
    ...
    rev = julie_list_value(interp);

    for (i = (long long)julie_array_len(list->list) - 1; i >= 0; i -= 1) {
        it = julie_array_elem(list->list, i);
        JULIE_ARRAY_PUSH(rev->list, julie_force_copy(interp, it));
    }
...
static Julie_Status julie_builtin_any(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *fn;
    Julie_Value  *list;
    Julie_Value  *it;
    ...
    *result = NULL;
    ...
    ARRAY_FOR_EACH(list->list, it) {
        ...
        truth = (ev->type == JULIE_SINT) ? !!ev->sint : !!ev->uint;
        julie_free_value(interp, ev);
        if (truth) {
            *result = julie_sint_value(interp, 1);
            goto out_pop;
...
static Julie_Status julie_builtin_all(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *fn;
    Julie_Value  *list;
    Julie_Value  *it;
...
static Julie_Status julie_builtin_zip(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status        status;
    Julie_Value        *a;
    Julie_Value        *b;
    Julie_Value        *zipped;
    ...
    *result = NULL;
    ...
    if (a->type != JULIE_LIST) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[0], JULIE_LIST, a->type);
        goto out_free;
    }

    if (b->type != JULIE_LIST) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[1], JULIE_LIST, b->type);
        goto out_free;
    }

    len = julie_array_len(a->list);
    if (julie_array_len(b->list) < len) {
        len = julie_array_len(b->list);
    }

    zipped = julie_list_value(interp);

    for (i = 0; i < len; i += 1) {
        ea   = julie_array_elem(a->list, i);
        eb   = julie_array_elem(b->list, i);
        pair = julie_list_value(interp);
        JULIE_ARRAY_PUSH(pair->list, julie_force_copy(interp, ea));
        JULIE_ARRAY_PUSH(pair->list, julie_force_copy(interp, eb));
        JULIE_ARRAY_PUSH(zipped->list, pair);
    }
...
static Julie_Status julie_builtin_concat(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *out_list;
    unsigned      i;
    Julie_Value  *ev;
    Julie_Value  *it;
    ...
    status   = JULIE_SUCCESS;
    out_list = julie_list_value(interp);

    for (i = 0; i < n_values; i += 1) {
        status = julie_eval(interp, values[i], &ev);
        if (status != JULIE_SUCCESS) {
            julie_free_value(interp, out_list);
            *result = NULL;
            goto out;
        }
        ...
        if (ev->type != JULIE_LIST) {
            status = JULIE_ERR_TYPE;
            julie_make_type_error(interp, values[i], JULIE_LIST, ev->type);
            julie_free_value(interp, ev);
            julie_free_value(interp, out_list);
            *result = NULL;
            goto out;
        ...
        ARRAY_FOR_EACH(ev->list, it) {
            JULIE_ARRAY_PUSH(out_list->list, julie_force_copy(interp, it));
        }
...
static Julie_Status julie_builtin_pair(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    ...
    *result = NULL;
    ...
    status = julie_eval_no_throw(interp, values[0], &first);
    if (status != JULIE_SUCCESS) {
        goto out;
    }

    status = julie_eval_no_throw(interp, values[1], &second);
    if (status != JULIE_SUCCESS) {
...
check_for_rewritten:;
            ...
            status = julie_eval_no_throw(interp, julie_array_elem(it->list, val_idx), &val);
            if (status != JULIE_SUCCESS) {
                julie_free_value(interp, key);
                goto out_free_object;
            }
...
static Julie_Status _julie_builtin_field(Julie_Interp *interp, Julie_Value *expr, Julie_Value *object, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    ...
    if (!JULIE_TYPE_IS_KEYLIKE(key->type)) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[0], _JULIE_KEYLIKE, key->type);
        *result = NULL;
        goto out_free_key;
    }
    ...
    if (field == NULL) {
        status = JULIE_ERR_BAD_INDEX;
        julie_make_bad_index_error(interp, values[0], key);
        *result = NULL;
        goto out_free_key;
    } else {
...
static Julie_Status julie_builtin_get_or_insert_field(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    ...
    *result = NULL;
    ...
    if (lookup != NULL) {
        lookup->owned = o->owned;
        *result = julie_copy(interp, lookup);
        goto out_free;
    }


    status = julie_eval_no_throw(interp, values[2], &val);
    if (status != JULIE_SUCCESS) {
        *result = NULL;
        julie_free_value(interp, key);
        goto out_free;
...
static Julie_Status julie_builtin_select(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    ...
    status = julie_eval_no_throw(interp, then, &then);
    if (status != JULIE_SUCCESS) {
        *result = NULL;
        goto out_free_cond;
    }
    ...
    for (;;) {
        ...
        for (i = 1; i < n_values; i += 1) {
            ...
            if (i < n_values - 1) {
                ...
                status = julie_eval_no_throw(interp, _val, &val);
            }
        ...
        ARRAY_FOR_EACH(container->list, it) {
            ...
            for (j = 2; j < n_values; j += 1) {
                ...
                if (j < n_values - 1) {
                    ...
                    status = julie_eval_no_throw(interp, val, &ev);
                }

                if (status != JULIE_SUCCESS) {
                    JULIE_UNBORROW(bound);
                    if (interp->local_symtab_depth == 0) {
    ...
    for (i = 1; i < n_values; i += 1) {
        ...
        for (j = 1; j < julie_array_len(list->list); j += 1) {
            if (j < julie_array_len(list->list) - 1) {
                ...
                status = julie_eval_no_throw(interp, julie_array_elem(list->list, j), result);
            }
...
static Julie_Status julie_builtin_join(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status        status;
    Julie_Value        *list;
    Julie_Value        *sep;
    const char         *csep;
    ...
    *result = NULL;
    ...
    status = julie_eval(interp, values[1], &sep);
    if (status != JULIE_SUCCESS) {
        julie_free_value(interp, list);
        goto out;
    }
    ...
    if (sep->type != JULIE_STRING) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[1], JULIE_STRING, sep->type);
        goto out_free;
    }

    csep    = julie_value_cstring(sep);
    sep_len = sep->tag == JULIE_STRING_TYPE_INTERN
                ? julie_get_string(interp, sep->string_id)->len
                : strlen(csep);
    ...
    ARRAY_FOR_EACH(list->list, it) {
        if (it->type != JULIE_STRING) {
            status = JULIE_ERR_TYPE;
            julie_make_type_error(interp, it, JULIE_STRING, it->type);
            free(str);
            goto out_free;
        }

        cs     = julie_value_cstring(it);
        cs_len = it->tag == JULIE_STRING_TYPE_INTERN
                    ? julie_get_string(interp, it->string_id)->len
                    : strlen(cs);

        if (!first) {
            str = realloc(str, str_len + sep_len + 1);
            memcpy(str + str_len, csep, sep_len);
            str_len += sep_len;
            str[str_len] = 0;
        }

        str = realloc(str, str_len + cs_len + 1);
        memcpy(str + str_len, cs, cs_len);
        str_len += cs_len;
        str[str_len] = 0;
...
out_free:;
    julie_free_value(interp, list);
    julie_free_value(interp, sep);

out:;
    return status;
}

static Julie_Status julie_builtin_upper(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *s;
    char         *str;
    char         *p;
    ...
    *result = NULL;
    ...
    str = strdup(julie_value_cstring(s));
    for (p = str; *p; p += 1) {
        if (*p >= 'a' && *p <= 'z') { *p -= 32; }
    }
...
static Julie_Status julie_builtin_lower(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *s;
    char         *str;
    char         *p;
...
static Julie_Status julie_builtin_sfind(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *haystack;
    Julie_Value  *needle;
    const char   *h;
    ...
    *result = NULL;
    ...
    status = julie_eval(interp, values[0], &haystack);
    if (status != JULIE_SUCCESS) {
        goto out;
    }

    if (haystack->type != JULIE_STRING) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[0], JULIE_STRING, haystack->type);
        julie_free_value(interp, haystack);
        goto out;
    }

    status = julie_eval(interp, values[1], &needle);
    if (status != JULIE_SUCCESS) {
        julie_free_value(interp, haystack);
        goto out;
    }

    if (needle->type != JULIE_STRING) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[1], JULIE_STRING, needle->type);
        julie_free_value(interp, haystack);
        julie_free_value(interp, needle);
        goto out;
    }
    ...
    *result = found != NULL
                ? julie_sint_value(interp, found - h)
                : julie_nil_value(interp);
...
static Julie_Status julie_builtin_fopen_ap(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    return _julie_builtin_fopen(interp, expr, n_values, values, result, "a");
}
...
static Julie_Status julie_builtin_fread(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *file;
    Julie_Value  *nv;
    Julie_Value  *key;
    ...
    *result = NULL;
    ...
    status = julie_eval(interp, values[0], &file);
    if (status != JULIE_SUCCESS) {
        goto out;
    }

    if (file->type != JULIE_OBJECT) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[0], JULIE_OBJECT, file->type);
        julie_free_value(interp, file);
        goto out;
    }

    status = julie_eval(interp, values[1], &nv);
    if (status != JULIE_SUCCESS) {
        julie_free_value(interp, file);
        goto out;
    }

    if (!JULIE_TYPE_IS_INTEGER(nv->type)) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[1], _JULIE_INTEGER, nv->type);
        julie_free_value(interp, file);
        julie_free_value(interp, nv);
        goto out;
    }
    ...
    f = (handle != NULL && handle->type == JULIE_UINT) ? (void*)handle->uint : NULL;
    n = (nv->type == JULIE_SINT) ? nv->sint : (long long)nv->uint;
    ...
    *result = julie_string_value_known_size(interp, buf, r);
    free(buf);
...
static Julie_Status julie_builtin_fexists(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *pathv;
    const char   *path;
    ...
    *result = NULL;
    ...
    status = julie_eval(interp, values[0], &pathv);
    if (status != JULIE_SUCCESS) {
        goto out;
    }

    if (pathv->type != JULIE_STRING) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[0], JULIE_STRING, pathv->type);
        julie_free_value(interp, pathv);
        goto out;
    }

    path    = julie_value_cstring(pathv);
    *result = julie_sint_value(interp, access(path, F_OK) == 0);
...
static Julie_Status julie_builtin_fremove(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *pathv;
    ...
    *result = julie_sint_value(interp, remove(julie_value_cstring(pathv)) == 0);
    julie_free_value(interp, pathv);
...
static Julie_Status julie_builtin_frename(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *oldv;
    Julie_Value  *newv;
    ...
    *result = NULL;
    ...
    status = julie_eval(interp, values[0], &oldv);
    if (status != JULIE_SUCCESS) {
        goto out;
    }

    if (oldv->type != JULIE_STRING) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[0], JULIE_STRING, oldv->type);
        julie_free_value(interp, oldv);
        goto out;
    }

    status = julie_eval(interp, values[1], &newv);
    if (status != JULIE_SUCCESS) {
        julie_free_value(interp, oldv);
        goto out;
    }

    if (newv->type != JULIE_STRING) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[1], JULIE_STRING, newv->type);
        julie_free_value(interp, oldv);
        julie_free_value(interp, newv);
        goto out;
    }

    *result = julie_sint_value(interp, rename(julie_value_cstring(oldv), julie_value_cstring(newv)) == 0);
    julie_free_value(interp, oldv);
    julie_free_value(interp, newv);
...
static void julie_sandbox_error_handler(Julie_Error_Info *info) {
    ...
    if (info->status != JULIE_ERR_ERROR_VALUE) {
        ...
        error_value = julie_error_value(info->interp, s);
        free(s);
        julie_free_error_info(info);
        info->status = JULIE_ERR_ERROR_VALUE;
        info->thrown.error_value = error_value;
    }
    memcpy(&info->interp->sandbox_error_info, info, sizeof(info->interp->sandbox_error_info));
}

static Julie_Status julie_builtin_parse_julie(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status          status;
    Julie_Value          *code;
    const char           *code_cstr;
    unsigned long long    code_len;
    Julie_Error_Callback  save_err_callback;
    ...
    *result = NULL;
    ...
    code_cstr = julie_value_cstring(code);
    code_len  = code->tag == JULIE_STRING_TYPE_INTERN
                    ? julie_get_string(interp, code->string_id)->len
                    : strlen(code_cstr);

    save_err_callback = interp->error_callback;
    save_file         = interp->cur_file_id;

    julie_set_cur_file(interp, julie_get_string_id(interp, "<parse-julie>"));
    interp->error_callback = julie_sandbox_error_handler;

    status = julie_parse_roots(interp, &roots, code_cstr, (int)code_len, &err_line, &err_col, 1);

    if (status != JULIE_SUCCESS) {
        julie_make_parse_error(interp, err_line, err_col, status);
        status = JULIE_SUCCESS;
        *result = julie_force_copy(interp, interp->sandbox_error_info.thrown.error_value);
        julie_free_error_info(&interp->sandbox_error_info);
        memset(&interp->sandbox_error_info, 0, sizeof(interp->sandbox_error_info));
        goto out_restore;
    }

    if (julie_array_len(roots) == 1) {
        *result = julie_array_elem(roots, 0);
    } else {
        do_list = julie_list_value(interp);
        do_sym  = julie_symbol_value(interp, julie_get_string_id(interp, "do"));
    ...
    roots = JULIE_ARRAY_INIT;

out_restore:;
    interp->error_callback = save_err_callback;
...
static Julie_Status julie_parse_sandbox_config(Julie_Interp *interp, Julie_Value *expr, Julie_Value *config, Julie_Value **out_bindings, Julie_Value **out_blacklist) {
    Julie_Value  *key;
    Julie_Value **lookup;
    ...
    *out_blacklist = NULL;
    ...
    if (config == NULL) { return JULIE_SUCCESS; }

    key    = julie_symbol_value(interp, julie_get_string_id(interp, "'bindings"));
    lookup = hash_table_get_val((_Julie_Object)config->object, key);
    julie_free_value(interp, key);
    if (lookup != NULL) {
        if ((*lookup)->type != JULIE_OBJECT) {
            julie_make_type_error(interp, expr, JULIE_OBJECT, (*lookup)->type);
            return JULIE_ERR_TYPE;
        }
        *out_bindings = julie_copy(interp, *lookup);
    }

    key    = julie_symbol_value(interp, julie_get_string_id(interp, "'blacklist"));
    lookup = hash_table_get_val((_Julie_Object)config->object, key);
    julie_free_value(interp, key);
    if (lookup != NULL) {
        if ((*lookup)->type != JULIE_LIST) {
            if (*out_bindings) { julie_free_value(interp, *out_bindings); *out_bindings = NULL; }
            julie_make_type_error(interp, expr, JULIE_LIST, (*lookup)->type);
            return JULIE_ERR_TYPE;
        }
        *out_blacklist = julie_copy(interp, *lookup);
    }

    return JULIE_SUCCESS;
}

static int julie_is_blacklisted(const char *name, Julie_Value *blacklist) {
    Julie_Value *it;

    if (blacklist == NULL) { return 0; }

    ARRAY_FOR_EACH(blacklist->list, it) {
        if ((it->type == JULIE_STRING || it->type == JULIE_SYMBOL)
        &&  strcmp(julie_value_cstring(it), name) == 0) {
...
static Julie_Array *julie_detach_bindings_from_object(Julie_Value *obj) {
    Julie_Array             *ret = JULIE_ARRAY_INIT;
    Julie_Value             *sym;
    Julie_Value            **valp;
    Julie_Detached_Binding  *new_binding;

    hash_table_traverse((_Julie_Object)obj->object, sym, valp) {
        if (sym->type != JULIE_SYMBOL && sym->type != JULIE_STRING) { continue; }

        new_binding        = malloc(sizeof(*new_binding));
        new_binding->name  = strdup(julie_value_cstring(sym));
        new_binding->value = julie_detach_value(*valp);

        JULIE_ARRAY_PUSH(ret, new_binding);
    }

    return ret;
}

static Julie_Array *julie_detach_globals(Julie_Interp *interp, Julie_Value *blacklist) {
    Julie_Array             *ret = JULIE_ARRAY_INIT;
    Julie_Symbol_Table      *symtab;
    unsigned                 i;
    const char              *name;
    ...
    if (symtab->expansion == NULL) {
        for (i = 0; i < symtab->used; i += 1) {
            name = julie_get_cstring(symtab->syms[i]);
            if (julie_is_blacklisted(name, blacklist)) { continue; }

            new_binding        = malloc(sizeof(*new_binding));
            new_binding->name  = strdup(name);
            new_binding->value = julie_detach_value(symtab->vals[i]);

            JULIE_ARRAY_PUSH(ret, new_binding);
        }
    } else {
        hash_table_traverse(symtab->expansion, sym, valp) {
            name = julie_get_cstring(sym);
            if (julie_is_blacklisted(name, blacklist)) { continue; }
...
static void julie_configure_sandbox(Julie_Interp *sandbox, Julie_Interp *parent, const char *file_name) {
    memset(&sandbox->sandbox_error_info, 0, sizeof(sandbox->sandbox_error_info));

    if (parent->output_callback != NULL) {
        julie_set_output_callback(sandbox, parent->output_callback);
    }
#ifdef JULIE_ENABLE_EVAL_CALLBACKS
    if (parent->eval_callback != NULL) {
        julie_set_eval_callback(sandbox, parent->eval_callback);
    }
    if (parent->post_eval_callback != NULL) {
        julie_set_post_eval_callback(sandbox, parent->post_eval_callback);
    }
#endif
    julie_set_error_callback(sandbox, julie_sandbox_error_handler);
    julie_set_cur_file(sandbox, julie_get_string_id(sandbox, file_name));
}

static void julie_bind_detached_array(Julie_Interp *sandbox, Julie_Array *dbindings) {
    Julie_Detached_Binding *dbinding;
    Julie_String_ID         id;
    Julie_Value            *attached_value;

    ARRAY_FOR_EACH(dbindings, dbinding) {
        id             = julie_get_string_id(sandbox, dbinding->name);
        attached_value = julie_attach_value(sandbox, dbinding->value);
        julie_bind(sandbox, id, &attached_value);

        free(dbinding->name);
        free(dbinding);
    }

    julie_array_free(dbindings);
}

static Julie_Value *julie_sandbox_exec(Julie_Interp *sandbox, Julie_Value *detached_expr, Julie_Status *out_status, Julie_Value **out_error_value) {
    Julie_Value  *attached;
    Julie_Status  status;
    Julie_Value  *result;
    Julie_Value  *detached;

    *out_error_value = NULL;
    detached         = NULL;

    attached = julie_attach_source_value(sandbox, detached_expr);

    status = julie_eval(sandbox, attached, &result);
    ...
    if (result != NULL) {
        detached = julie_detach_value(result);
        julie_free_value(sandbox, result);
    }

done:;
    julie_free_source_node(sandbox, attached);

    *out_status = status;
    if (status != JULIE_SUCCESS) {
        *out_error_value = julie_detach_value(sandbox->sandbox_error_info.thrown.error_value);
    }

    return detached;
}

static Julie_Actor *julie_actor_new(void);
...
static Julie_Status julie_builtin_eval_sandboxed(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    ...
    Julie_Value  *blacklist;
    Julie_Interp *sandbox;
    Julie_Array  *dbindings;
    Julie_Value  *detached;
    Julie_Value  *dval;
    Julie_Status  sandbox_status;
    Julie_Value  *detached_error_value;
    ...
    *result = NULL;
    ...
    if (code->type != JULIE_LIST) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[0], JULIE_LIST, code->type);
        goto out_free_code;
    }
    ...
    blacklist = NULL;
    if (n_values == 2) {
        status = julie_eval(interp, values[1], &config);
        if (status != JULIE_SUCCESS) { goto out_free_code; }

        if (config->type != JULIE_OBJECT) {
            status = JULIE_ERR_TYPE;
            julie_make_type_error(interp, values[1], JULIE_OBJECT, config->type);
            goto out_free_config;
        }

        status = julie_parse_sandbox_config(interp, expr, config, &bindings, &blacklist);
        if (status != JULIE_SUCCESS) { goto out_free_config; }
    }

    (void)blacklist;

    sandbox = julie_init_sandboxed_interp();
    sandbox->actor = julie_actor_new();

    julie_configure_sandbox(sandbox, interp, "<eval-sandboxed>");

    if (bindings != NULL) {
        dbindings = julie_detach_bindings_from_object(bindings);
        julie_bind_detached_array(sandbox, dbindings);
    }

    dval     = julie_detach_value(code);
    detached = julie_sandbox_exec(sandbox, dval, &sandbox_status, &detached_error_value);

    julie_free_error_info(&sandbox->sandbox_error_info);
    julie_free(sandbox);

    if (sandbox_status != JULIE_SUCCESS) {
        *result = julie_attach_value(interp, detached_error_value);
    } else if (detached != NULL) {
        *result = julie_attach_value(interp, detached);
    } else {
        *result = julie_nil_value(interp);
    }
...
out_free_config:;
    if (blacklist != NULL) { julie_free_value(interp, blacklist); }
    if (bindings  != NULL) { julie_free_value(interp, bindings);  }
    if (config    != NULL) { julie_free_value(interp, config);    }
...
static void julie_mailbox_init(Julie_Mailbox *mb) {
    pthread_mutex_init(&mb->lock, NULL);
    pthread_cond_init(&mb->cond, NULL);
    mb->queue  = JULIE_ARRAY_INIT;
    mb->closed = 0;
}

static void julie_mailbox_destroy(Julie_Mailbox *mb) {
    Julie_Value *it;
    ARRAY_FOR_EACH(mb->queue, it) {
        julie_free_detached_value(it);
    }
    julie_array_free(mb->queue);
    pthread_cond_destroy(&mb->cond);
    pthread_mutex_destroy(&mb->lock);
}

static void julie_mailbox_close(Julie_Mailbox *mb) {
...
static int julie_mailbox_send(Julie_Mailbox *mb, Julie_Value *detached) {
    int closed;
    ...
    if (!closed) {
        JULIE_ARRAY_PUSH(mb->queue, detached);
...
static Julie_Value *julie_mailbox_receive(Julie_Mailbox *mb) {
    Julie_Value *msg;

    pthread_mutex_lock(&mb->lock);

    while (julie_array_len(mb->queue) == 0 && !mb->closed) {
        pthread_cond_wait(&mb->cond, &mb->lock);
    }

    if (julie_array_len(mb->queue) == 0) {
        pthread_mutex_unlock(&mb->lock);
        return NULL;
    }

    msg = julie_array_elem(mb->queue, 0);
    julie_array_erase(mb->queue, 0);
...
static Julie_Value *julie_mailbox_try_receive(Julie_Mailbox *mb) {
    Julie_Value *msg;
...
static void julie_actor_ref(Julie_Actor *actor) {
    __sync_fetch_and_add(&actor->rc, 1);
}
...
static void julie_actor_unref(Julie_Actor *actor) {
    ...
    rc = __sync_sub_and_fetch(&actor->rc, 1);
    if (rc > 0) { return; }

    if (!actor->is_main && !actor->joined) {
        pthread_detach(actor->thread);
    }

    julie_mailbox_destroy(&actor->mailbox);

    if (actor->detached_result != NULL) {
        julie_free_detached_value(actor->detached_result);
    }
    if (actor->detached_error_value != NULL) {
        julie_free_detached_value(actor->detached_error_value);
    }

    ARRAY_FOR_EACH(actor->bindings, dbinding) {
        free(dbinding->name);
        julie_free_detached_value(dbinding->value);
        free(dbinding);
    }
    julie_array_free(actor->bindings);

    if (actor->detached_expr != NULL) {
        julie_free_detached_value(actor->detached_expr);
        actor->detached_expr = NULL;
    }

    if (actor->parent != NULL) {
        julie_actor_unref(actor->parent);
    }
...
static Julie_Actor *julie_actor_new(void) {
    ...
    actor->stop     = 0;
    actor->parent   = NULL;
    actor->children = JULIE_ARRAY_INIT;
    pthread_mutex_init(&actor->children_mtx, NULL);
...
static Julie_Actor *julie_actor_new_main(void) {
    Julie_Actor *actor;

    actor = julie_actor_new();
...
static Julie_Value *julie_actor_value(Julie_Interp *interp, Julie_Actor *actor) {
    Julie_Value *v;
    ...
    julie_actor_ref(actor);

    return v;
}


static void *julie_actor_thread_entry(void *arg) {
    Julie_Actor     *actor;
    Julie_Interp    *interp;
    Julie_String_ID  parent_id;
    Julie_Value     *parent_handle;
    Julie_Value     *detached;
    Julie_Status     sandbox_status;
    Julie_Value     *detached_error_value;
    unsigned         i;
    ...
    actor = (Julie_Actor*)arg;

    interp = julie_init_sandboxed_interp();
    interp->actor = actor;

    if (actor->output_callback != NULL) {
        julie_set_output_callback(interp, actor->output_callback);
    }

    memset(&interp->sandbox_error_info, 0, sizeof(interp->sandbox_error_info));
    julie_set_error_callback(interp, julie_sandbox_error_handler);
    julie_set_cur_file(interp, julie_get_string_id(interp, "<actor>"));

    julie_bind_detached_array(interp, actor->bindings);
    ...
    detached = julie_sandbox_exec(interp, actor->detached_expr, &sandbox_status, &detached_error_value);
    actor->detached_expr = NULL;

    actor->final_status         = sandbox_status;
    actor->detached_result      = detached;
    actor->detached_error_value = detached_error_value;

    julie_free_error_info(&interp->sandbox_error_info);

    actor->alive = 0;
    julie_mailbox_close(&actor->mailbox);

    /* Remove ourself from parent's children list, but only on normal exit.
       If stop is set, the parent is holding children_mtx and will
       clear the list itself. */
    if (actor->parent != NULL && !actor->stop) {
        pthread_mutex_lock(&actor->parent->children_mtx);
        i = 0;
        ARRAY_FOR_EACH(actor->parent->children, child) {
            if (child == actor) {
                julie_array_erase(actor->parent->children, i);
                julie_actor_unref(actor);
                break;
            }
            i += 1;
        }
        pthread_mutex_unlock(&actor->parent->children_mtx);
    }
...
static Julie_Status julie_builtin_actor_spawn(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status            status;
    Julie_Value            *code;
    Julie_Value            *config;
    Julie_Value            *bindings;
    Julie_Value            *blacklist;
    Julie_Actor            *actor;
    Julie_Array            *dbindings       = JULIE_ARRAY_INIT;
    Julie_Array            *dbindings_extra = JULIE_ARRAY_INIT;
    Julie_Detached_Binding *dbinding;
    int                     err;
    ...
    *result = NULL;
    ...
    dbindings = julie_detach_globals(interp, blacklist);

    if (bindings != NULL) {
        dbindings_extra = julie_detach_bindings_from_object(bindings);

        ARRAY_FOR_EACH(dbindings_extra, dbinding) {
            JULIE_ARRAY_PUSH(dbindings, dbinding);
        }
        julie_array_free(dbindings_extra);
    }

    actor = julie_actor_new();

    actor->parent = interp->actor;
    actor->rc += 1; /* parent's handle */
    julie_actor_ref(actor->parent);
    ...
    pthread_mutex_lock(&interp->actor->children_mtx);
    JULIE_ARRAY_PUSH(interp->actor->children, actor);
    julie_actor_ref(actor);
    pthread_mutex_unlock(&interp->actor->children_mtx);

    err = pthread_create(&actor->thread, NULL, julie_actor_thread_entry, actor);

    if (err != 0) {
        status = JULIE_ERR_ACTOR_JOIN;
    ...
    /* Create handle for parent.  actor starts at rc 2 (parent + child),
       and julie_actor_value bumps by 1 then unref drops back. */
    *result = julie_actor_value(interp, actor);
    julie_actor_unref(actor);
...
static Julie_Status julie_builtin_actor_send(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *handle;
    Julie_Value  *val;
    Julie_Actor  *target;
    ...
    *result = NULL;
    ...
    if (handle->type != JULIE_ACTOR) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[0], JULIE_ACTOR, handle->type);
        goto out_free_handle;
    }

    status = julie_eval(interp, values[1], &val);
    if (status != JULIE_SUCCESS) { goto out_free_handle; }

    target   = (Julie_Actor*)handle->object;
    detached = julie_detach_value(val);

    if (!julie_mailbox_send(&target->mailbox, detached)) {
        julie_free_detached_value(detached);

        /* Return an error value instead of throwing. */
        *result = julie_error_value(interp, julie_error_string(JULIE_ERR_ACTOR_DEAD));
        status = JULIE_SUCCESS;
        goto out_free_val;
    }
...
static Julie_Status julie_builtin_actor_receive(Julie_Interp *interp, Julie_Value  *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Value *detached;
    ...
    *result = NULL;
    ...
    if (n_values != 0) {
        julie_make_arity_error(interp, expr, 0, n_values, 0);
        return JULIE_ERR_ARITY;
    }

    if (interp->actor == NULL) {
        julie_make_interp_error(interp, expr, JULIE_ERR_NO_MAILBOX);
        return JULIE_ERR_NO_MAILBOX;
    }

    detached = julie_mailbox_receive(&interp->actor->mailbox);

    if (detached == NULL) {
        /* Mailbox closed — return error value. */
        *result = julie_error_value(interp, "Mailbox closed.");
    } else {
        *result = julie_attach_value(interp, detached);
    }
...
static Julie_Status julie_builtin_actor_try_receive(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Value  *detached;
    ...
    *result = NULL;
    ...
    detached = julie_mailbox_try_receive(&interp->actor->mailbox);
...
static Julie_Status julie_builtin_actor_join(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *handle;
    Julie_Actor  *actor;
    ...
    *result = NULL;
    ...
    actor = (Julie_Actor*)handle->object;
    ...
    if (actor->final_status != JULIE_SUCCESS) {
        *result = julie_attach_value(interp, actor->detached_error_value);
        actor->detached_error_value = NULL;
    } else if (actor->detached_result != NULL) {
        *result = julie_attach_value(interp, actor->detached_result);
        actor->detached_result = NULL;
    } else {
        *result = julie_nil_value(interp);
    }
...
static Julie_Status julie_builtin_actor_stop(Julie_Interp *interp, Julie_Value  *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *handle;
    Julie_Actor  *actor;
    Julie_Actor  *child;
...
static Julie_Status julie_builtin_eval_file(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    ...
    *result = NULL;
    ...
    status = julie_eval(interp, values[0], &path);
    if (status != JULIE_SUCCESS) { goto out; }

    if (path->type != JULIE_STRING) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[0], JULIE_STRING, path->type);
    ...
    pstring = julie_value_cstring(path);

    status = julie_map_file_into_readonly_memory(pstring, &mem, &size);
    if (status != JULIE_SUCCESS) {
        julie_make_file_error(interp, expr, status, pstring);
        status = JULIE_SUCCESS;
        *result = julie_force_copy(interp, interp->sandbox_error_info.thrown.error_value);
        julie_free_error_info(&interp->sandbox_error_info);
        memset(&interp->sandbox_error_info, 0, sizeof(interp->sandbox_error_info));
    ...
    interp->cur_file_id = julie_get_string_id(interp, pstring);

    status = julie_parse_roots(interp, &roots, mem, size, &err_line, &err_col, 1);
    if (status != JULIE_SUCCESS) {
        julie_make_parse_error(interp, err_line, err_col, status);
        status = JULIE_SUCCESS;
        *result = julie_force_copy(interp, interp->sandbox_error_info.thrown.error_value);
...
static Julie_Status julie_builtin_getenv(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *name;
    const char   *val;
    ...
    *result = NULL;
    ...
    if (name->type != JULIE_STRING) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[0], JULIE_STRING, name->type);
        julie_free_value(interp, name);
        goto out;
    }

    val     = getenv(julie_value_cstring(name));
    *result = val != NULL ? julie_string_value(interp, val) : julie_nil_value(interp);
    julie_free_value(interp, name);
...
static Julie_Status julie_builtin_setenv(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *name;
    Julie_Value  *val;
    ...
    *result = NULL;
    ...
    if (val->type != JULIE_STRING) {
        status = JULIE_ERR_TYPE;
        julie_make_type_error(interp, values[1], JULIE_STRING, val->type);
        julie_free_value(interp, name);
        julie_free_value(interp, val);
        goto out;
    }

    setenv(julie_value_cstring(name), julie_value_cstring(val), 1);
...
static Julie_Status julie_builtin_sleep(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status  status;
    Julie_Value  *f;
...
static Julie_Status julie_builtin_time(Julie_Interp *interp, Julie_Value *expr, unsigned n_values, Julie_Value **values, Julie_Value **result) {
    Julie_Status    status;
    struct timespec ts;
    ...
    *result = julie_uint_value(interp, (unsigned long long)ts.tv_sec * 1000000000ULL + (unsigned long long)ts.tv_nsec);
    ...
    switch (fn->type) {
        ...
        case JULIE_LAMBDA:
            ...
            for (i = 0; i < n_params - !!vargs; i += 1) {
                status = julie_eval_no_throw(interp, values[i], &ev);
...
static Julie_Status _julie_eval(Julie_Interp *interp, Julie_Value *value, Julie_Value **result, int throw_err) {
    Julie_Status             status;
    ...
    *result    = NULL;
    ...
    if (unlikely(interp->actor->stop)) {
        julie_make_interp_error(interp, value, JULIE_ERR_EVAL_CANCELLED);
    ...
    if (value->type == JULIE_LIST) {
        ...
        if (unlikely(list_len <= 1)) {
            ...
            &&  !source_info->paren) {
                ...
                value = julie_array_elem(value->list, 0);
            } else if (list_len == 0) {
    ...
    if (interp->eval_callback != NULL) {
        status = interp->eval_callback(interp, value, result);
        if (status != JULIE_SUCCESS) {
...
out:;
    if (throw_err && status == JULIE_SUCCESS && *result != NULL && (*result)->type == JULIE_ERROR) {
        julie_make_runtime_error(interp, orig_value, *result);
    ...
    if (interp->post_eval_callback != NULL && status == JULIE_SUCCESS) {
        status = interp->post_eval_callback(interp, status, value, result);
        if (status != JULIE_SUCCESS) {
            julie_make_interp_error(interp, orig_value, status);
            goto out;
        }
...
static Julie_Status julie_eval_no_throw(Julie_Interp *interp, Julie_Value *value, Julie_Value **result) {
    return _julie_eval(interp, value, result, 0);
}
...
#define JULIE_BIND_INFIX_FN(_name, _fn) julie_bind_infix_fn(interp, julie_get_string_id(interp, (_name)), (_fn))
    ...
    JULIE_BIND_FN(      "error",                 julie_builtin_error);
    JULIE_BIND_FN(      "error-field",           julie_builtin_error_field);

    JULIE_BIND_INFIX_FN("=",                     julie_builtin_assign);
    JULIE_BIND_INFIX_FN(":=",                    julie_builtin_assign_global);
    JULIE_BIND_FN(      "unbind",                julie_builtin_unbind);
    ...
    JULIE_BIND_FN(      "pow",                   julie_builtin_pow);
    JULIE_BIND_FN(      "abs",                   julie_builtin_abs);
    JULIE_BIND_FN(      "sqrt",                  julie_builtin_sqrt);
    JULIE_BIND_FN(      "floor",                 julie_builtin_floor);
    JULIE_BIND_FN(      "ceil",                  julie_builtin_ceil);
    JULIE_BIND_FN(      "round",                 julie_builtin_round);
    JULIE_BIND_FN(      "log",                   julie_builtin_log);
    JULIE_BIND_FN(      "log2",                  julie_builtin_log2);
    JULIE_BIND_FN(      "log10",                 julie_builtin_log10);
    JULIE_BIND_FN(      "exp",                   julie_builtin_exp);
    JULIE_BIND_FN(      "sin",                   julie_builtin_sin);
    JULIE_BIND_FN(      "cos",                   julie_builtin_cos);
    JULIE_BIND_FN(      "tan",                   julie_builtin_tan);
    JULIE_BIND_FN(      "asin",                  julie_builtin_asin);
    JULIE_BIND_FN(      "acos",                  julie_builtin_acos);
    JULIE_BIND_FN(      "atan",                  julie_builtin_atan);
    JULIE_BIND_FN(      "atan2",                 julie_builtin_atan2);
    JULIE_BIND_FN(      "clamp",                 julie_builtin_clamp);
    ...
    JULIE_BIND_FN(      "map",                   julie_builtin_map);
    JULIE_BIND_FN(      "filter",                julie_builtin_filter);
    JULIE_BIND_FN(      "reduce",                julie_builtin_reduce);
    JULIE_BIND_FN(      "reverse",               julie_builtin_reverse);
    JULIE_BIND_FN(      "any",                   julie_builtin_any);
    JULIE_BIND_FN(      "all",                   julie_builtin_all);
    JULIE_BIND_FN(      "zip",                   julie_builtin_zip);
    JULIE_BIND_FN(      "concat",                julie_builtin_concat);
    ...
    JULIE_BIND_FN(      "join",                  julie_builtin_join);
    JULIE_BIND_FN(      "upper",                 julie_builtin_upper);
    JULIE_BIND_FN(      "lower",                 julie_builtin_lower);
    JULIE_BIND_FN(      "sfind",                 julie_builtin_sfind);
    ...
    JULIE_BIND_FN(      "fopen-ap",              julie_builtin_fopen_ap);
    JULIE_BIND_FN(      "fclose",                julie_builtin_fclose);
    JULIE_BIND_FN(      "frewind",               julie_builtin_frewind);
    JULIE_BIND_FN(      "fread",                 julie_builtin_fread);
    ...
    JULIE_BIND_FN(      "fexists",               julie_builtin_fexists);
    JULIE_BIND_FN(      "fremove",               julie_builtin_fremove);
    JULIE_BIND_FN(      "frename",               julie_builtin_frename);

    JULIE_BIND_FN(      "backtrace",             julie_builtin_backtrace);

    JULIE_BIND_FN(      "parse-julie",           julie_builtin_parse_julie);
    JULIE_BIND_FN(      "eval-file",             julie_builtin_eval_file);
    JULIE_BIND_FN(      "eval-sandboxed",        julie_builtin_eval_sandboxed);

    JULIE_BIND_FN(      "actor-spawn",           julie_builtin_actor_spawn);
    JULIE_BIND_FN(      "actor-send",            julie_builtin_actor_send);
    JULIE_BIND_FN(      "actor-receive",         julie_builtin_actor_receive);
    JULIE_BIND_FN(      "actor-try-receive",     julie_builtin_actor_try_receive);
    JULIE_BIND_FN(      "actor-join",            julie_builtin_actor_join);
    JULIE_BIND_FN(      "actor-stop",            julie_builtin_actor_stop);
    ...
    JULIE_BIND_FN(      "getenv",                julie_builtin_getenv);
    JULIE_BIND_FN(      "setenv",                julie_builtin_setenv);
    JULIE_BIND_FN(      "sleep",                 julie_builtin_sleep);
    JULIE_BIND_FN(      "time",                  julie_builtin_time);

    if (!sandboxed) {
        interp->actor = julie_actor_new_main();
    }
...
void julie_free(Julie_Interp *interp) {
    ...
    pthread_mutex_lock(&interp->actor->children_mtx);
    ARRAY_FOR_EACH(interp->actor->children, child) {
        child->stop = 1;
        julie_mailbox_close(&child->mailbox);
    }
    ARRAY_FOR_EACH(interp->actor->children, child) {
    ...
    ARRAY_FOR_EACH(interp->actor->children, child) {
        julie_actor_unref(child);
    }
    julie_array_free(interp->actor->children);
    pthread_mutex_unlock(&interp->actor->children_mtx);
    ...
    if (interp->actor != NULL) {
        interp->actor->alive = 0;
        julie_mailbox_close(&interp->actor->mailbox);
        julie_actor_unref(interp->actor);
        interp->actor = NULL;
    }

    it = interp->value_free_list;
    while (it != NULL) {
        next = it->free_list_next;
        free(it);
        it = next;
    }
